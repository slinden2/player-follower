...
const PlayerContextProvider = props => {
  ...
  const [followPlayer, { followState }] = useMutation(FOLLOW_PLAYER, {
    ...
    update: (store, response) => {
      ...
      if (response.data?.FollowPlayer) {
        const userWithNewFav = {
          ...dataInStore.me,
          favoritePlayers: dataInStore.me.favoritePlayers.concat(
            response.data.FollowPlayer._id
          ),
        }

        store.writeQuery({ query: USER, data: { me: userWithNewFav } })
      }
    },
  })
  ...
  const [unfollowPlayer, unfollowState] = useMutation(UNFOLLOW_PLAYER, {
    ...
    update: (store, response) => {
      ...
      if (response.data?.FollowPlayer) {
        const userWithoutFav = {
          ...dataInStore.me,
          favoritePlayers: dataInStore.me.favoritePlayers.filter(
            id => id !== response.data.FollowPlayer._id
          ),
        }

        store.writeQuery({ query: USER, data: { me: userWithoutFav } })
      }
    },
  })
